...
Luckily that is easy -- we can sort in reverse order using the `-` operator inside the `ntile()` function:
...
On the other hand, if we pick too many, we may end up with groups that are indistinguishable and hard to explain to managers, like in the exercise above.
