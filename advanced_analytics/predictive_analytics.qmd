...
Accurate forecasts reduce labour costs by avoiding overstaffing, prevent service bottlenecks by avoiding understaffing, and ultimately improve both customer and employee satisfaction.

## The Setting: Attendance at Major League Baseball Games {.unnumbered}

To see these ideas in action, we turn to an example from professional sports. Stadium operators must staff concession stands that sell food and beverages during baseball games. If they schedule too few employees, service slows and queues lengthen; too many, and labour costs surge.

Their challenge begins with a prediction problem: how many fans will attend each game? Attendance depends on many factors—day of the week, weather, opponent quality, promotional events, and more. Once attendance is forecast, managers can estimate how many customers will visit concession stands per hour and allocate staff accordingly.
...
- [ ] Use insights from exploratory analysis to fit prediction models and make forecasts: lagged values, trailing averages, and cumulative averages
...
A simple example is `bref_team_results()`, a function that retrieves all games played by a given MLB team during a particular season. For instance, the code below pulls the 2015 game results for the Boston Red Sox:
...
Note that some seasons have fewer than 162 games because of cancellations caused by poor weather, labour disputes, and other factors.
...
A few date strings failed to parse—often corresponding to doubleheader games (two games played on the same day). We can inspect them directly:
...
# handling doubleheader annotations like "(1)" / "(2)" and extra whitespace.
bos_2005_2015_fenway <-
  bos_2005_2015_fenway |>
  mutate(
    ymd = date |>
      str_remove(".*, ") |>            # drop weekday and trailing comma (e.g., "Sunday, ")
      str_remove("\\s*\\(\\d+\\)") |>  # remove "(1)" or "(2)" from doubleheaders
...
For those doubleheaders, we also want to flag them explicitly.
...
Now, let's check for these doubleheaders:
...
Next, we will create a variable that identifies the day of the week on which each game was played. This will be useful later, as attendance often varies by day of the week (e.g., weekends vs. weekdays). We call this variable `dotw`:
...
Our next step will be to explore what factors might drive the variation shown in this histogram.
...
#### Model 1: Persistence (Naïve Time-Series Model) {.unnumbered}

A natural starting point is the assumption that attendance at a game will be roughly the same as attendance at the previous game. This is known as a persistence forecast or naïve forecast, and it’s the foundation for many time-series methods.
...
ggplot(bos_2005_2015_fenway,
  aes(x = att_avg_lag3, y = attendance)) +
  geom_point(color = "darkblue", alpha = 0.3) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "red") +
  labs(
...
Operationally, this bias risks overstaffing when demand is low (wasted labour costs) and understaffing when demand is high (long queues and worse service at precisely the wrong time). To improve, use a shorter-memory signal (e.g., lag-1 or a short moving average, or exponential smoothing) and incorporate forward-looking features such as day/night, day of week, opponent, ticket sales, and weather; also cap forecasts at stadium capacity to respect the ceiling.
...
- Attendance at the previous game is highly informative — crowds don’t fluctuate dramatically from one homestand to the next.
...
The Lag(1) model provides the most accurate predictions, but it is not likely to be practical for operations planning. Forecasts based purely on the previous game may not give managers enough lead time to hire staff, order supplies, or adjust logistics.
...
In this week's workshop we will turn to building models that incorporate these rich, forward-looking predictors.
...
- Overall, modest structure that tracks stable temporal patterns beats heavy flexibility that chases noise.
