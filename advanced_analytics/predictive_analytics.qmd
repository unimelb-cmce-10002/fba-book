...
* Summarize and visualize attendance distributions; recognize capacity-driven censoring (sell-outs) and describe implications for feasible forecast ranges.
* Build baseline predictors (lag-1, trailing-3, cumulative mean) and simple cell-means models (Day/Night; dotw × Day/Night; Opponent) with a time-based train/test split to avoid look-ahead bias.
* Fit and compare OLS models (factor model, quadratic in game number, high-order polynomial) and explain overfitting vs. generalization.
...
By the end of this week's module, you will be able to take a data set like this:
...
![Comment here...](../figs/forecast_metrics_by_model.png){width=100% fig-align="center"}
...
- [ ] Use insights from exploratory analysis to fit prediction models and make forecasts: baselines, groups-means, and OLS
...
In these notes we focus on regular-season home games for one team. The variables you’ll see—opponent (`Opp`), home/away (`H_A`), day vs night (`D/N`), date/weekday (`dotw`), and attendance—align with operational drivers (for example, weekend nights and rivalry opponents often draw larger crowds). Weather, promotions, and team performance can also shift demand, while stadium capacity imposes a natural upper bound on forecasts.
...
This produces a tidy data set with both teams, across all selected years, each record tagged with Team and Season:
...
We can fix these manually by assigning the correct dates:
...
Next, we will create a variable that identifies the day of the week on which each games was played. This will be useful later, as attendance often varies by day of the week (e.g., weekends vs. weekdays). We do so by `dotw`:
...
Next, we filter to games where the Red Sox played at home. We so because in our analyses we are taking on the perspective of the firm that manages concessions at Fenway Park, the Red Sox home stadium. Thus, we only care about games where the Red Sox are the home team:
...
5. **Operational takeaway**  
   ...
   - Risky when conditions change; pair with forward-looking features (day/night, DOTW, opponent, weather) and judge by MAE/RMSE, not just the plot.
...
Q1 — Low-actual games (≈30–33k): Where do points sit relative to the 45° line, and what does that imply about prediction error?
Q2 — High-actual games (≈38k+): Where do points sit relative to the 45° line, and what does that imply?
Q3 — Bias: What overall bias does this model exhibit, and what visual cues in the plot support your claim?
Q4 — Operations: What are the operational consequences of this bias for staffing concessions?
Q5 — Improving the model: Describe two changes to the approach that could reduce these errors.
...
- Create a data frame cummulative_hist_model_errors with the columns: error (actual − predicted), abs_error, squared_error, and percentage_error (in %).
- Summarise these into cummulative_hist_model_metrics containing MAE, RMSE, and MAPE.
...
# Summarise error diagnostics into accuracy metrics
cummulative_hist_model_metrics <-
  cummulative_hist_model_errors %>%
  summarise(
    # Mean Absolute Error
    MAE  = mean(abs_error, na.rm = TRUE),
...
ts_model_metrics <-
  ...
  bind_rows(cummulative_hist_model_metrics)
...
### Judging model perfromance against a random baseline
...
bos_2005_2015_fenway <-
  bos_2005_2015_fenway %>%
  mutate(
    # Draw one random prediction per game from N(36,500, 2,000^2)
    att_rnorm = rnorm(n(), mean = 36500, sd = 2000)
  )
