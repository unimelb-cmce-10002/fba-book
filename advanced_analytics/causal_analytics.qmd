...
To see how each of these options plays out in practice, let's turn to an example.  
...
In general, business analysts separate word of mouth into offline and online:
...
- If a donor who saw the new message instead saw the old one, what would have happened?  
- If a donor who saw the old message instead saw the new one, what would have happened?   
...
So $Y_i(1)$ means "what happens to donor $i$ if they get treatment," while $Y_i(0)$ means "what happens if they don't." The causal effect for donor $i$ is the difference between these two potential outcomes:  
...
Since we can't calculate the causal effect for one donor, we shift our focus to groups. 
...
where $\bar{Y}(1)$ is the average sharing rate among donors who actually saw the new message, and $\bar{Y}(0)$ is the average sharing rate among donors who saw the old one. This is known as the **Sample Average Treatment Effect** or, more simply, the **difference in means estimator**.

So does that mean we can just take the difference in sample means and be done? 
Not quite. 
The formula is correct — the average treatment effect can be estimated as a difference in means — but only if we are comparing the **appropriate groups** and comparing the **appropriate sample means**.
...
To see how this works, let's abstract away from the DonorsChoose business problem and start with a fictitious example where we are in control of how the data are generated. 
Imagine we're studying a very simple artificial economy where people earn an income.
...
We want to use these data to estimate the causal effect of having brown hair on income.
...
What if we tease out the mean difference in the data by comparing the mean income of brown-haired people to non-brown-haired people?
Computing the difference in log income will give us a percentage difference:
...
That might feel like an odd statement because we are comparing across hair colors, which is what we wanted to do.
...
The question is whether we can make assumptions or use what we know about the economy to find the appropriate comparisons and **recover** the causal effect of brown hair on income from the data.
...
This effect size is in the direction we expect and much closer to the 10% that we know is true. The difference between 12.2% and 10% is due to randomness.
...
If our model is right (and this is a big if), our inference is credible.
...
Instead of leaning on assumptions about how the world behaves, it looks to **the way data were generated and collected** — i.e. the *design* — to provide the basis for causal analysis. 
We're going to focus on situations where the design of the data collection is intentional and the data-generating process is controlled by the analyst. 
...
So far, we've talked about experiments where *we* decide who gets which version of a message, offer, policy, or experience. 
...
The key idea behind design-based inference is that we can *create* fair comparisons through how we generate and collect data.
...
That's why we ideally want to compare the characteristics of the treatment and control groups and verify that any differences in composition are just random noise.
...
By creating comparable groups *before* data collection, randomization makes the "appropriate sample means" appear — by design, not by luck or modeling.
...
By comparing means across conditions, we can observe whether the difference in messaging led to (a) more users sharing, and (b) more recruitment of new donors:
...
Let's think of another way to establish whether the campaign was effective:
...
- **Design-based inference** relies on how data are generated and collected.
