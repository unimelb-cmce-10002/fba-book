...
sales_plot_final <-
    ggplot(beer) +
    geom_density(aes(x = log(qty), fill = imported), 
                 alpha = 0.35
                 ) +
    scale_fill_okabe_ito() 
...
Recall our workflow (borrowed from Rohan Alexander for now):
...
Remark: Perhaps we leave (2) out until we get to "Deeper Insights" or it disappears altogether

Sketch of the plan: 3 plots:
...
Note this prints the plot, can store for later
...
Create a boxplot comparing the prices across different beer brands
...
ggplot(beer) +
    ...
    geom_smooth(aes(y = log(price), x = log(qty)), 
                method = "lm", 
                color =  "#CC79A7"
                ) +
    facet_wrap(~ imported)
